 ...
 * `organizationId` and filters by it — callers (server actions) resolve the
 * active org first. Kept free of session/redirect logic so it's testable
	...
	await db.transaction(async (tx) => {
		...
		if (serviceIds.length > 0) {
			await tx
				...
				.values(
					serviceIds.map((serviceId) => ({ professionalId: id, serviceId }))
				);
		}
...
export async function updateProfessional(
	...
	await db.transaction(async (tx) => {
		const updated = await tx
			...
			.set({ name: input.name, calendarId: input.calendarId || null })
			.where(
				and(
