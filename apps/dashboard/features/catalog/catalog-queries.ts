...
/**
 * Wrapped in React `cache()` because two components on `/profissionais` read it
 * in the same render — the table (to name each professional's services) and the
 * "novo profissional" dialog (to offer the checkboxes). One request, one query.
 */
export const listServices = cache(
	(orgId: string): Promise<ServiceRecord[]> =>
		db
			.select({
